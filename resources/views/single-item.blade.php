    ...
    <div class="container py-md-5">
        ...
        <div class="d-flex justify-content-between">
          ...
          <span class="pt-2">
            ...
            <a href="/item/{{$item->id}}/edit" class="text-primary mr-2" data-toggle="tooltip" data-placement="top" title="Edit"><i class="fas fa-edit"></i></a>      
        ...
        <div class="row">
            <div class="col-md-6">
                <p><strong>Found Date:</strong> {{$item->lost_date}}</p>
                <p><strong>Category:</strong> {{$item->category}}</p>
                <p><strong>Location:</strong> {{$item->location}}</p>
                <p style="margin-bottom: 4px;"><strong>Claimed By:</strong> {{ $item->claimed_by }}</p>
                @if (
                      $item->status !== 'Claimed' &&
                ...
                @endif
                <strong>Posted by:</strong> <a href="{{ url('/profile/' . $item->user->username) }}">{{ $item->user->username }}</a>
            ...
            <div class="col-md-6">
                ...
                <p><strong>Storage Bin:</strong> {{$item->bin}}</p>
                <p><strong>Issued By:</strong> {{$item->issued_by}}</p>
                <p><strong>Issued Date:</strong> {{$item->issued_date}}</p>
                <p><strong>Received By: {{$item->received_by}}</strong> </p>
                <p><strong>Received Date: {{$item->received_date}}</strong> </p>
        ...
        <div class="body-content">
            <p><strong>Markings:</strong> <textarea readonly class="form-control mb-2" rows="3" readonly style="height: 50px; width: 100%; overflow-y: auto; background-color: #e2e8f0; border: none; resize: none; padding: 5px; border-radius: 5px;">{{ $item->markings }}</textarea></p>
